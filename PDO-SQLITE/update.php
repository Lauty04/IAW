...
/*         foreach ($result as $campo => $valor) {
        ...
        <form method="post" action="<?php echo $_SERVER['PHP_SELF']. "?id=$id" ?>">
            <table border="1">
                ...
                <tr>
                    ...
                    <td><input type="text" name="telefono" pattern="\+?\d{1,12}" title="Ingresa un número de teléfono válido" value="<?php echo $ptelf_contacto; ?>"></td>
                    <td>
                        <select name="enfermedad">
                            ...
                            $result = $stmt->setFetchMode(PDO::FETCH_ASSOC);
                            foreach ($stmt->fetchAll() as $row) {
                            $selected = ($row['nombre_enfermdedad'] == $penfermedad) ? 'selected' : ''; 
    ...
    <?php
        ...
        $stmt = $conn->prepare("SELECT `codigo`,`nombre_enfermedad` FROM `enfermedades` ");
        $stmt->execute();
        $result = $stmt->setFetchMode(PDO::FETCH_ASSOC);
        foreach ($stmt->fetchAll() as $row) {
            if ($enfermedad == $row['nombre_enfermedad']){
