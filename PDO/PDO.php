...
<body>
    ...
    <div class="boton">
        <h2><a href="PDOinsert.php" >INSERTAR</a></h2>
    </div>
    <div class="principal">
    <?php
    $stmt = $conn->prepare("SELECT `id_paciente`,`nombre`,`apellido1`,`apellido2`,`genero`,`f_nacimiento`,`cod_post`,`telf_contacto`,`cod_enfermedad` FROM `paciente`");
    $stmt->execute();
    ...
    foreach ($stmt->fetchAll() as $row) {
        ...
        echo "<td>{$row['cod_enfermedad']}</td>";
        echo "<td>
                <a href='modificar.php?id={$row['id_paciente']}' target='_blank' class='modificar-btn'>MODIFICAR</a>
    ...
    <script>
        ...
        Array.from(eliminarButtons).forEach(function (button) {
            button.addEventListener('click', function (event) {
                ...
                if (confirmacion) {
                    window.location.href = 'eliminar.php?id=' + pacienteID;
                }
            });
        });
